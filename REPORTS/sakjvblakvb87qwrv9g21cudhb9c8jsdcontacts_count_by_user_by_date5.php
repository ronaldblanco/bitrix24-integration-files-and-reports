...
//var_dump(date("Y-m-d h:i:s",strtotime(date("Y-m-d h:i:s") . ' +7 hour')));
...
CONVERT_TZ(b_crm_contact.DATE_CREATE,'+00:00','-05:00') >= '".$from."%' AND CONVERT_TZ(b_crm_contact.DATE_CREATE,'+00:00','-05:00') <='".$to3."%' AND SOURCE_ID = 18 INNER JOIN 
b_user ON b_user.ID = b_crm_contact.ASSIGNED_BY_ID INNER JOIN b_user_index ON
 b_user_index.USER_ID = b_user.ID WHERE b_user.WORK_NOTES = 'ext' AND b_user.WORK_NOTES = 'ext/sp' GROUP BY ASSIGNED_BY_ID ORDER BY Date asc, COUNT(ID) asc limit 250;");
*/
...
INNER JOIN b_user ON b_user.ID = b_crm_contact.ASSIGNED_BY_ID AND b_uts_crm_contact.UF_CRM_1591040450352 LIKE '%i:167%' INNER JOIN b_user_index ON
 b_user_index.USER_ID = b_user.ID GROUP BY UserID limit 250");
...
foreach($period as $date){
	$filename= '../APP/logs/total_ext_'.$date->format("m.d.y").'.txt';
...
<body>
	...
	<center><h2 class = "margin-top">Leads Control Report from Zapier -External Department-</h2>

	<form action="https://crm.api.com/bitrix/admin/dhhdbw73723934dvrgintegration/REPORTS/sakjvblakvb87qwrv9g21cudhb9c8jsdcontacts_count_by_user_by_date5.php" method="POST">
<div class="container">
  <div class="row">
    <div class="col-md-9">
</div>
