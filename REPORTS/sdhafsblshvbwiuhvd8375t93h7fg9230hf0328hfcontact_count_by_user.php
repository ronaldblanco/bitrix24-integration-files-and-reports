...
$language = "English";
$users_group_len = DBGet("SELECT COUNT(ID),ASSIGNED_BY_ID FROM b_crm_contact INNER JOIN b_uts_crm_contact ON b_uts_crm_contact.VALUE_ID = b_crm_contact.ID and b_uts_crm_contact.UF_CRM_1590674689393 = '".$language."' AND UF_CRM_1591040450352 LIKE '%i:167;%' GROUP BY ASSIGNED_BY_ID ORDER BY COUNT(ID) LIMIT 500;"); //Query to database
		//var_dump($users_group_len[0]["ASSIGNED_BY_ID"]); //User with less contacts

$languageES = "Spanish";
$users_group_lenES = DBGet("SELECT COUNT(ID),ASSIGNED_BY_ID FROM b_crm_contact INNER JOIN b_uts_crm_contact ON b_uts_crm_contact.VALUE_ID = b_crm_contact.ID and b_uts_crm_contact.UF_CRM_1590674689393 = '".$languageES."' AND UF_CRM_1591040450352 LIKE '%i:167;%' GROUP BY ASSIGNED_BY_ID ORDER BY COUNT(ID) LIMIT 500;"); //Query to database
		//var_dump($users_group_len[0]["ASSIGNED_BY_ID"]); //User with less contacts
...
<body>
	...
	<center><h2>Contacts Count by User!</h2></center>
