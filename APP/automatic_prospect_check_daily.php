...
//$date = date("Y-m-d");
$date = date("Y-m-d",strtotime(date("Y-m-d h:i:s") . ' +7 hour'));
...
$no_prospects_by_day = DBGet("select b_crm_contact.ID from b_crm_contact INNER JOIN b_uts_crm_contact ON b_uts_crm_contact.VALUE_ID = b_crm_contact.ID where CONVERT_TZ(b_crm_contact.DATE_CREATE,'+00:00','-05:00') LIKE '".$date."%' and b_uts_crm_contact.UF_CRM_1591040450352 not LIKE '%i:167;%' limit 1000;");
...
foreach($prospects as $contact){
	...
	//var_dump($responsablemanager['manager']['ID']);
